...
describe('calculateIncomeAllocation', () => {
  ...
  it('counts covered scheduled payments inside the configured debt share', () => {
    const result = calculateIncomeAllocation(30000, 6011, 0, 60, 25, 15, true)

    expect(result.scheduledApplied).toBe(6011)
    expect(result.freshDebtBudget).toBe(1489)
    expect(result.debtBudget).toBe(1489)
    expect(result.btcAmount).toBe(18000)
    expect(result.cashAmount).toBe(4500)
  ...
  it('caps early payments by total debt without subtracting scheduled payments twice', () => {
    const result = calculateIncomeAllocation(10000, 500, 0, 60, 25, 15, true, {
      totalDebtBalanceCzk: 1000,
      withoutDebtBtcPercent: 80,
      withoutDebtCashPercent: 20,
    })

    expect(result.scheduledApplied).toBe(500)
    expect(result.debtBudget).toBe(1000)
