...
describe('VwcePage', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn().mockImplementation(async (input, init) => {
      ...
      if (url.endsWith('/vgla/overview')) return {
        ok: true,
        status: 200,
        json: async () => ({
          totals: { shares: 0.0139, costBasisEur: 2.1416, accountCount: 1, costBasisComplete: true, provisionalLotCount: 0, rentRatePercent: 2, rentPoolCzk: 40 },
          accounts: [{
            id: 'xtb', name: 'XTB', description: null, ownerDisplayName: 'Samuel', shares: 0.0139,
            costBasisEur: 2.1416, costBasisComplete: true, lotCount: 1, disposalCount: 0,
          ...
          recentMovements: [{
            ...
            unitPriceEur: 154.10, proceedsEur: null, occurredAt: '2026-05-07T10:00:00Z', note: null,
          }],
        }),
      } as Response
      if (url.endsWith('/vgla-price')) return {
        ok: true,
        status: 200,
        json: async () => ({ priceEur: 159.96, priceCzk: 3999, eurCzk: 25, observedAt: '2026-08-21T10:00:00Z', source: 'yahoo-finance', isStale: false }),
      } as Response
      if (url.endsWith('/accounts/xtb/movements')) return { ok: true, status: 200, json: async () => [{
        ...
        unitPriceEur: 154.10, proceedsEur: null, occurredAt: '2026-05-07T10:00:00Z', note: 'Nákup na XTB',
        canEdit: true, canDelete: true,
      }] } as Response
      if (url.endsWith('/identity/antiforgery')) return { ok: true, status: 200, json: async () => ({ token: 'csrf-token' }) } as Response
      if (url.endsWith('/vgla/accounts')) return { ok: true, status: 201, json: async () => ({ id: 'new-account' }) } as Response
      if (url.endsWith('/vgla/payouts')) return { ok: true, status: 201, json: async () => ({ id: null, requestedAmountCzk: 10, amountCzk: 0, deferred: true, rentPoolCzk: 50 }) } as Response
      if (url.endsWith('/vgla/accounts/xtb/purchases')) return { ok: true, status: 201, json: async () => ({ id: 'new-purchase' }) } as Response
      if (url.endsWith('/default-share')) return { ok: true, status: 204 } as Response
      if (url.endsWith('/vgla/accounts/xtb')) return init?.method === 'DELETE'
        ? { ok: true, status: 204 } as Response
        : { ok: true, status: 200, json: async () => ({ id: 'xtb', name: 'XTB upraveno' }) } as Response
      if (url.endsWith('/vgla/movements/account-lot/purchase')) return { ok: true, status: 204 } as Response
  ...
  it('renders owner-scoped VGLA totals, broker and rent controls', async () => {
    const user = userEvent.setup()
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const router = createTestRouter('/vgla')
    await router.load()
    render(<QueryClientProvider client={queryClient}><RouterProvider router={router} /></QueryClientProvider>)

    expect(await screen.findByRole('heading', { name: 'VGLA' })).toBeInTheDocument()
    ...
    expect(within(purchaseDialog).getByLabelText('Cena za podíl (EUR)')).toHaveValue('159.9600')
    await user.click(within(purchaseDialog).getByRole('button', { name: 'Přidat nákup' }))
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([input, options]) => String(input).endsWith('/vgla/accounts/xtb/purchases') && options?.method === 'POST')).toBe(true))
    await user.click(screen.getByRole('button', { name: 'Upravit pohyb' }))
    await user.click(screen.getByRole('button', { name: 'Uložit pohyb' }))
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([input, options]) => String(input).endsWith('/vgla/movements/account-lot/purchase') && options?.method === 'PUT')).toBe(true))
    ...
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([input, options]) => String(input).endsWith('/vgla/movements/account-lot/purchase') && options?.method === 'DELETE')).toBe(true))
    ...
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([input, options]) =>
      String(input).endsWith('/vgla/accounts/xtb') && options?.method === 'PUT')).toBe(true))
    ...
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([input, options]) =>
      String(input).endsWith('/vgla/accounts/xtb') && options?.method === 'DELETE')).toBe(true))
  ...
  it('creates a broker account and submits a rent payout from header actions', async () => {
    ...
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([input, options]) =>
      String(input).endsWith('/vgla/accounts') && options?.method === 'POST')).toBe(true))
    ...
    await waitFor(() => expect(vi.mocked(fetch).mock.calls.some(([input, options]) => {
      if (!String(input).endsWith('/vgla/payouts') || options?.method !== 'POST') return false
