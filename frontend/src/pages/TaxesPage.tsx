...
export function TaxesPage() {
  ...
  return <section className="taxes-page">
    ...
    {confirming && <div className="dialog-backdrop" role="presentation"><section className="taxes-confirm" role="dialog" aria-modal="true" aria-label="Odložit realizaci do VGLA"><button type="button" aria-label="Zavřít" onClick={() => setConfirming(false)}><X size={17} /></button><span>VGLA MÍSTO BTC</span><h2>Odložit {czk.format(data.recommendedTransferCzk)}?</h2><p>BTC se neprodá. Checkpoint se upraví, jako by realizace proběhla, a stejná částka se uloží do poolu pro budoucí nákupy VGLA.</p><label>Poznámka<input value={note} maxLength={500} onChange={(event) => setNote(event.target.value)} /></label>{defer.error && <p className="form-error" role="alert">{defer.error.message}</p>}<div><button type="button" onClick={() => setConfirming(false)}>Zrušit</button><button className="primary" type="button" disabled={defer.isPending} onClick={() => defer.mutate()}>{defer.isPending ? 'Odkládám…' : 'Potvrdit odložení'}</button></div></section></div>}
  </section>
}
