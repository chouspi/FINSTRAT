...
describe('SettingsPage', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn().mockImplementation(async (input) => {
      ...
      const strategySettings = { btcTaxPeriodYears: 3, checkpointTriggerFloorCzk: 20000, checkpointTriggerPercent: 10, realizationStepProfitCzk: 20000, realizationStepTransferCzk: 10000, vwceRentRatePercent: 2 }
  ...
  it('opens and saves BTC strategy settings in its own tab', async () => {
    ...
    expect(screen.getByText(/Checkpoint je aktivní automaticky/)).toBeInTheDocument()
    expect(screen.queryByLabelText('Práh aktivace checkpointu')).not.toBeInTheDocument()
