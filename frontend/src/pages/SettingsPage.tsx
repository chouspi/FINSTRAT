...
function StrategySettingsForm({ initial }: { initial: StrategySettings }) {
  ...
  return <div className="settings-pane"><SettingsHeading eyebrow="BTC STRATEGIE" title="Checkpoint a realizace zisku" description="Checkpoint je aktivní automaticky. Parametry určují profit trigger a doporučenou částku pro přesun do VWCE." valid={valid} save={save} /><div className="strategy-settings-card">{fields.map(([label, key, unit, min, step]) => <label className="strategy-setting-row" key={key}><span>{label}</span><span className="strategy-number-control"><input aria-label={label} type="number" min={min} max={key === 'btcTaxPeriodYears' ? 20 : undefined} step={step} value={settings[key] as number} onChange={(event) => setSettings({ ...settings, [key]: Number(event.target.value) })} /><b>{unit}</b></span></label>)}</div>{!valid && <p className="settings-error">Zkontrolujte rozsahy a kladné částky strategie.</p>}{save.error && <p className="settings-error" role="alert">{save.error.message}</p>}</div>
}
