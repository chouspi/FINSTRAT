...
type IncomeSettings = { defaultCapitalCzk: number; withoutDebtBtcPercent: number; withoutDebtCashPercent: number; withDebtBtcPercent: number; withDebtDebtPercent: number; withDebtCashPercent: number }
...
function IncomeSettingsPane() {
  ...
  return <IncomeAllocationSettings initial={overview.data.settings} />
}

function IncomeAllocationSettings({ initial }: { initial: IncomeSettings }) {
  const queryClient = useQueryClient()
  const [settings, setSettings] = useState(initial)
  const valid = settings.withoutDebtBtcPercent + settings.withoutDebtCashPercent === 100 && settings.withDebtBtcPercent + settings.withDebtDebtPercent + settings.withDebtCashPercent === 100
  const save = useMutation({ mutationFn: async () => { const token = await antiforgeryToken(); return apiRequest<IncomeSettings>('/api/income-plan/settings', { method: 'PUT', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token }, body: JSON.stringify({ ...settings, defaultCapitalCzk: String(settings.defaultCapitalCzk) }) }) }, onSuccess: async (saved) => { setSettings(saved); await queryClient.invalidateQueries({ queryKey: ['income-plan'] }) } })
  return <div className="settings-pane"><SettingsHeading eyebrow="INCOME PLÁN" title="Pravidla rozdělení příjmu" description="Procenta se automaticky použijí podle toho, zda máte aktivní spotřebitelský dluh." valid={valid} save={save} /><div className="settings-profiles"><AllocationProfile title="Bez spotřebitelských dluhů" fields={[["BTC", 'withoutDebtBtcPercent'], ["Cash", 'withoutDebtCashPercent']]} settings={settings} onChange={setSettings} /><AllocationProfile title="S aktivními dluhy" fields={[["BTC", 'withDebtBtcPercent'], ["Dluhy", 'withDebtDebtPercent'], ["Cash", 'withDebtCashPercent']]} settings={settings} onChange={setSettings} /></div>{!valid && <p className="settings-error">Každý profil musí dávat přesně 100 %.</p>}{save.error && <p className="settings-error" role="alert">{save.error.message}</p>}</div>
}
...
function AllocationProfile({ title, fields, settings, onChange }: { title: string; fields: [string, keyof IncomeSettings][]; settings: IncomeSettings; onChange: (settings: IncomeSettings) => void }) {
