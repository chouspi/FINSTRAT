...
function StrategySettingsForm({ initial }: { initial: StrategySettings }) {
  const queryClient = useQueryClient()
  const [settings, setSettings] = useState(initial)
  const valid = settings.btcTaxPeriodYears >= 1 && settings.btcTaxPeriodYears <= 20 && settings.checkpointTriggerFloorCzk >= 0 && settings.checkpointTriggerPercent >= 0 && settings.checkpointTriggerPercent <= 100 && settings.realizationStepProfitCzk > 0 && settings.realizationStepTransferCzk > 0 && settings.vwceRentRatePercent >= 0 && settings.vwceRentRatePercent <= 100
  ...
  const fields: [string, keyof StrategySettings, string, number, number][] = [['Spodní hranice triggeru', 'checkpointTriggerFloorCzk', 'Kč', 0, 1000], ['Trigger jako podíl checkpointu', 'checkpointTriggerPercent', '%', 0, 1], ['Realizace za každých', 'realizationStepProfitCzk', 'Kč zisku', 1, 1000], ['Převést do VGLA', 'realizationStepTransferCzk', 'Kč', 1, 1000], ['Sazba renty VGLA', 'vwceRentRatePercent', '% p.a.', 0, .1], ['Daňový časový test BTC', 'btcTaxPeriodYears', 'roky', 1, 1]]
  return <div className="settings-pane"><SettingsHeading eyebrow="BTC STRATEGIE" title="Checkpoint a realizace zisku" description="Checkpoint je aktivní automaticky. Parametry určují profit trigger a doporučenou částku pro přesun do VGLA." valid={valid} save={save} /><div className="strategy-settings-card">{fields.map(([label, key, unit, min, step]) => <label className="strategy-setting-row" key={key}><span>{label}</span><span className="strategy-number-control"><input aria-label={label} type="number" min={min} max={key === 'btcTaxPeriodYears' ? 20 : undefined} step={step} value={settings[key] as number} onChange={(event) => setSettings({ ...settings, [key]: Number(event.target.value) })} /><b>{unit}</b></span></label>)}</div>{!valid && <p className="settings-error">Zkontrolujte rozsahy a kladné částky strategie.</p>}{save.error && <p className="settings-error" role="alert">{save.error.message}</p>}</div>
}
